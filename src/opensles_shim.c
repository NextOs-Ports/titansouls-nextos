...
/* Initialize SDL2 audio. A completed attempt is successful only while the
 * real SDL device remains open; a failed attempt is latched for this engine
 * lifetime so OpenSL cannot manufacture players without audio.output-open. */
static int ensure_audio_initialized(void) {
  if (g_audio_initialized)
    return g_audio_dev != 0 ? 0 : -1;
...
static SLresult engine_CreateAudioPlayer(void *self, void **pPlayer,
  ...
  if (pPlayer)
    *pPlayer = NULL;
  if (ensure_audio_initialized() != 0) {
    logPrintf("[audio] CreateAudioPlayer recusado: output/receipt indisponivel\n");
